...
void Sakura::Inject()
{
    ...
    if (Injector::GetProcessInfo("cstrike.exe", &processInfo) == false && moduleExists)
    {
        sprintf(statusText, /*cstrike.exe is missing!*/XorStr<0x82, 19, 0x7A22A4AD>("\xEA\xEF\xAA\xE0\xFE\xE2\xA8\xE0\xF9\xAB\xE1\xE4\xFD\xFC\xF9\xFF\xF5\xB2" + 0x7A22A4AD).s);
        processExists = false;
    }

    if (Injector::InjectModule("cstrike.exe", modulePath) == true && moduleExists && processExists)
    {
